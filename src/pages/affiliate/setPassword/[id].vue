...
const route = useRoute('affiliate-setPassword-id')
    ...
    refVForm.value?.validate().then(async ({ valid: isValid }) => {
        ...
        if (isValid) {
            ...
            try {
                ...
                await affiliateStore.savePassword({ token: token, password: password.value });
                router.push({ name: 'affiliate-login' })
                        ...
                        <VRow>
                            ...
                            <VCol cols="12">
                                <VTextField v-model="password" :label="$t('Password')" placeholder="············"
                                    :rules="[requiredValidator, setPasswordValidator]" :error-messages="errors.password"
                                    autofocus :type="isPasswordVisible ? 'text' : 'password'"
