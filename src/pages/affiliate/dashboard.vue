...
definePage({
  meta: {
    action: 'read',
    subject: 'Affiliate Dashboard View',
  },
})
...
const logisticData = ref([
  ...
  },
  {
    ...
    color: 'info',
    title: 'Affiliate Weekly Earnings',
    value: 0,
    change: 0,
    isHover: false,
    footerText: 'than last week'
...
account_number.value = affiliateStore.affiliateAccount?.account_number ?? null;
routing_number.value = affiliateStore.affiliateAccount?.routing_number ?? null;
ach.value = affiliateStore.affiliateAccount?.ach ?? null;
bank_name.value = affiliateStore.affiliateAccount?.bank_name ?? null;
account_type.value = affiliateStore.affiliateAccount?.account_type ?? null;
onMounted(async () => {
  ...
  await affiliateStore.getAffiliateDashboardData(query)
  // await store.dispatch('getAffiliateDashboardData', {
  //   start_date: startDate,
  //   end_date: endDate,
  //   affiliate_id: userData.value?.id
  // });
  logisticData.value[0].value = affiliateStore.dashbardStats.total_affiliate_active_patients??0;
  logisticData.value[1].value = affiliateStore.dashbardStats.total_weekly_appointments??0; 
  logisticData.value[2].value = formatCurrency(affiliateStore.dashbardStats.total_revenue??0); 
  logisticData.value[3].value = formatCurrency(affiliateStore.dashbardStats.weekly_revenue??0);
  series.value[0].data = affiliateStore.dashbardStats.line_chart.data
});
    ...
    <VCol cols="12" class="mt-4 mb-4">
      ...
      <div class="setup-card" v-if="!account_number || !routing_number || !ach || !account_type || !bank_name">
        ...
        <div class="setup-message">
          ...
          <span>
            {{ $t('Please set up your banking information to receive affiliate payments.') }}
            {{ $t('You need to add your banking details to get paid for your referrals.') }} 
            </span>
        </div>

        <router-link to="/affiliate/settings">
