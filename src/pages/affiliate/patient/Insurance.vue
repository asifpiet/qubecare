...
const onSubmit = async (trigger) => {
    ...
    return refForm.value?.validate().then(({ valid }) => {
        if (valid) {
            patientStore.saveAffiliateInsurance()
...
<template>
    <VForm ref="refForm" v-model="isFormValid" @submit.prevent="onSubmit">
        <VRow>
            ...
            <VCol cols="6">
                <AppTextField type="number" v-model="patientStore.insurance.insuredIDNumber"
                    :label="$t('Insured ID Number')" :placeholder="$t('Insured ID Number')" />
            </VCol>

            <!-- 👉 Username -->
            <VCol cols="6">
                <AppTextField v-model="patientStore.insurance.insuredGroupNameNo" :label="$t('Insured Group Name/No')"
                    :placeholder="$t('Insured Group Name/No')" />
            </VCol>

            <!-- 👉 Email -->
            <VCol cols="6">
                <AppTextField v-model="patientStore.insurance.employersSchoolName" :label="$t('Employers/School Name')"
                    :placeholder="$t('Employers/School Name')" />
            </VCol>
            ...
            <!-- 👉 Role -->
            <VCol cols="6">
                <AppSelect v-model="patientStore.insurance.subscriber_sex" :label="$t('Insured Gender')"
                    :placeholder="$t('Insured Gender')" :rules="[requiredValidator]"
            ...
            <VCol cols="12">
                <AppTextField v-model="patientStore.insurance.subscriber_employer_street" :rules="[requiredValidator]"
                    :label="$t('Insured Address')" :placeholder="$t('Insured Address')" />
            </VCol>

            <VCol cols="4">
                <AppTextField v-model="patientStore.insurance.subscriber_employer_city" :rules="[requiredValidator]"
                    :label="$t('Insured City')" :placeholder="$t('Insured City')" />
            </VCol>
            <!-- 👉 Plan -->
            <VCol cols="4">
                <AppSelect v-model="patientStore.insurance.subscriber_employer_state" :rules="[requiredValidator]"
                    :label="$t('Insured State')" :placeholder="$t('State')" :items="states" item-title="name"
                    item-value="abbreviation" />
            </VCol>
            <VCol cols="4">
                <AppTextField v-model="patientStore.insurance.insuredZip" :rules="[requiredValidator]"
                    :label="$t('Insured Zip')" :placeholder="$t('Insured Zip')" />
            </VCol>
            ...
            <VCol cols="6">
                <AppTextField v-model="patientStore.insurance.payerID" :label="$t('Payer ID')"
            ...
            <VCol cols="6">
                <AppTextField v-model="patientStore.insurance.referringProviderNPI"
                    :label="$t('Referring Provider NPI')" :placeholder="$t('Referring Provider NPI')
                        " />
            </VCol>
            <VCol cols="6">
