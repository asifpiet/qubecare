...
import { hideLoader } from '@/utils/showLoader';
...
const validatePayment = async () => {
    ...
    if (isValid) {
        ...
        try {
            ...
            let url = '/patient/schedule?bid=' + route.query.bid + '&cart=' + encodeBase64Unicode(patientRegStore.cart_products);
...
const saveOrderInfo = async () => {
    ...
    const productIds = products.map(product => ({
        product_id: product.site_product_id ? (product.wp_pro_id ?? product.id) : product.id,
        qty: product.qty,
    ...
    // Only process card details if payment is required
    let month = null;
    let year = null;

    if (getFinalAmount() > 0 && expiry.value) {
        [month, year] = expiry.value.split("/");
        year = year && year.length === 2 ? (year >= "50" ? "19" + year : "20" + year) : year;
    }
    ...
    let finalAmount = prescreptionRequired.value && practitioner_fee.value > 0 && !subscriptionRequired.value
        ? totals.value.doctorVisitTotal
        : totals.value.otherProductsTotal;

    // Apply discount if coupon is applied
    if (patientRegStore.appliedCoupon) {
        finalAmount = Math.max(0, finalAmount - patientRegStore.discountAmount);
    }

    // Prepare payment data object with conditional card details
    const paymentData = {
        ...
        items: productIds,
        affiliate_email: patientRegStore.patient.affiliate_email ?? null,
        prescreption_required: prescreptionRequired.value,
        practitioner_fee: typeof practitioner_fee.value === 'string' ? parseFloat(practitioner_fee.value) : practitioner_fee.value,
        provider_id: builderStore.provider_id,
        appointment_id: patientRegStore.appointment.id,
        wp_order_id: patientRegStore.wp_order_id,
        coupon_code: patientRegStore.appliedCoupon?.code || null,
        discount_amount: Number(patientRegStore.discountAmount.toFixed(2)) || 0
    };

    // Only add card details if payment is required
    if (getFinalAmount() > 0) {
        paymentData.card_number = cardNumber.value;
        paymentData.cvv = cvv.value;
        paymentData.expiration_month = month;
        paymentData.expiration_year = year;
    }

    console.log(paymentData);

    const formsArr = builderStore.layout.filter(item => item.type === 'forms' || item.type === 'comboforms');
    paymentData.pending_task = formsArr.length > 0 ? true : false;
    paymentData.builder_id = bid;

    await patientRegStore.saveOrder(paymentData);
}
...
const onEmailBlur = async () => {
    if (patientRegStore.currentState !== 'loggedin') {
        if (patientRegStore.patient.email)
            await patientRegStore.checkPatientEmail({ email: patientRegStore.patient.email, provider_id: builderStore.provider_id })
...
const login = async () => {
    ...
    if (patientRegStore.patient.email && patientRegStore.patient.password) {
        ...
        showLoader()
        try {
            let resp = await patientAuthService.login(patientRegStore.patient.email, patientRegStore.patient.password)
...
const signup = async () => {
    ...
    if (
        ...
        if (patientRegStore.currentState === 'register') {
            ...
            showLoader()
            try {
                let resp = await patientAuthService.register(patientRegStore.patient)
...
const calculateFinalPaymentAmount = () => {
    let amount = prescreptionRequired.value && practitioner_fee.value > 0 && !subscriptionRequired.value
        ? totals.value.doctorVisitTotal
        : totals.value.otherProductsTotal;

    // Apply discount if coupon is applied
    if (patientRegStore.appliedCoupon && patientRegStore.discountAmount) {
        amount = Math.max(0, amount - patientRegStore.discountAmount);
    }

    return productTotalPreReq(1, amount);
};
                            ...
                            <VRow :class="isMobile ? 'mt-5' : ''">
                                ...
                                <VCol cols="12" md="6" class="col-order-1" :class="isMobile ? '' : 'px-0 pr-10 pt-10'"
                                    ...
                                    <div class="mt-10">
                                        ...
                                        <div class="mb-3" :class="isMobile ? 'custom-card' : ''">
                                            <div v-if="getFinalAmount() > 0">
                                                <div>
                                                    ...
                                                    <small>{{ $t('Please provide your card details for payment.')
                                                    }}</small>
                                                </div>
                                                <VRow>
                                                    <VCol cols="12" lg="12" md="12">
                                                        <label>{{ $t('Card Number') }}</label>
                                                        <VTextField v-model="cardNumber" label=""
                                                            :rules="[requiredValidator, cardNumberValidator]"
                                                            placeholder="xxxx xxxx xxxx xxxx" @input="cardNumberFormat"
                                                            density="comfortable" />
                                                    </VCol>
                                                    <VCol cols="12" lg="6" md="6">
                                                        <label>{{ $t('Expiration Date') }}</label>
                                                        <VTextField v-model="expiry" label=""
                                                    ...
                                                    <VCol cols="12" lg="6" md="6">
                                                        <label>{{ $t('CVV') }}</label>
                                                        <VTextField v-model="cvv"
                                                            :rules="[requiredValidator, cvvValidator]" placeholder="xxx"
                                                            label="" maxlength="3" @input="handleCVVInput"
                                                            density="comfortable" />
                                                    </VCol>
                                                </VRow>
                                                <VRow>
                                                    <VCol cols="12" md="12">
                                                        <div class="card-icons">
                                                            <div class="card-icon">{{ $t('Visa') }}</div>
                                                            <div class="card-icon">{{ $t('MC') }}</div>
                                                            <div class="card-icon">{{ $t('Amex') }}</div>
                                                            <div class="card-icon">{{ $t('PayPal') }}</div>
                                                        </div>
                                                    </VCol>
                                                </VRow>
                                            </div>
                                            <VRow>
                                                <VCol cols="12" md="12">
                                                    <div class="disclaimer">
                                                        <span>
                                                            <VIcon class="custom-color">tabler-shield-check-filled
                                                            </VIcon>
                                                            &nbsp;
                                                            {{ $t('HIPAA-compliant') }}
                                                        </span>&nbsp;
                                                        <span>
                                                            <VIcon class="custom-color">tabler-lock-filled</VIcon>
                                                            &nbsp;
                                                            {{ $t('SSL') }}
                                                            {{ $t('Secure') }}
                                                            {{ $t('Payment') }}
                                                    ...
                                                    <p class="mt-6 disclaimer">{{ $t('Need assistance? Contact') }} <a
                                                            class="custom-color" :href="`mailto:${companyEmail}`">{{
                                            ...
                                            <VDialog v-model="confirmPopup" refs="myDialog" persistent width="500">
                                                <v-card>
                                                    ...
                                                    <v-card-actions>
                                                        <v-spacer></v-spacer>
                                                        <v-btn type="submit" :text="$t('Confirm')"
                                                            @click="validatePayment" :disabled="disableSubmit"></v-btn>
                                                        <v-btn :text="$t('Close')"
                                                            @click="confirmPopup = false"></v-btn>
                                                    </v-card-actions>
                                                </v-card>
                                            </VDialog>
                                        </div>
                                    ...
                                    <div class="mt-4" v-if="isMobile">
                                        <div class="text-center mb-2 mt-6">
                                            <VBtn class="px-4 mb-6" color="primary" variant="flat" block size="large"
                                                ...
                                                {{ $t('Confirm & Pay') }} {{ calculateFinalPaymentAmount() }}
                                            </VBtn>
