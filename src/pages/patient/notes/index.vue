...
watch(
    ...
    async (isOpen) => {
        if (isOpen) {
            ...
            patientStore.patientMessagesListBySubject = []
...
const loadSubjects = async () => {
    ...
    await patientStore.getPatientMessagesList(currentUser.value.id)
    subjectsList.value = patientStore.patientMessagesList
}
const reversedMessages = computed(() => {
    return [...(patientStore.patientMessagesListBySubject?.messages || [])].reverse()
})
const scrollToBottom = () => {
    setTimeout(() => {
        if (messageContainer.value) {
            const scrollElement = messageContainer.value.$el || messageContainer.value
            console.log(patientStore.patientMessagesListBySubject.messages.length)
            if (patientStore.patientMessagesListBySubject.messages.length > 2) {
...
const openSubjectMessages = async (subject) => {
    ...
    await patientStore.getPatientMessagBySubject(subject.id)
    scrollToBottom()
}
            ...
            <div class="subject-sidebar" style="width: 300px; border-right: 1px solid #e0e0e0;">
                ...
                <PerfectScrollbar class="subjects-scrollbar" :options="{ wheelPropagation: false }">
                    <VList lines="two" :select-strategy="'classic'">
                        <VListItem v-for="subject in subjectsList" :key="subject.id"
                            :active="activeSubject?.id === subject.id" @click="openSubjectMessages(subject)">
                            <VListItemTitle>
                                {{ subject.subject }}
                            </VListItemTitle>
                            <VListItemSubtitle>
                                {{ formatCustomDate(subject.date) }} • {{ subject.with_name }}
                ...
                <template v-if="activeSubject">
                    ...
                    <PerfectScrollbar ref="messageContainer" class="custom-scrollbar messages-container">
                        <div class="messages-content" style="height: calc(100vh - 54px);">
                            <template v-for="(msg, index) in patientStore.patientMessagesListBySubject?.messages"
                                :key="index">
                                        ...
                                        <div class="message-details text-caption mt-1 text-medium-emphasis">
                                            <div class="divider-container">
                                                ...
                                                <div class="details-content">
                                                    <span>{{ dateTimeFormat(msg.created_at, "MM-DD-YYYY hh:mm A")
                                                        }}</span>
                    ...
                    <!-- <div class="message-input px-4 py-3 border-top mb-3">
                        <VForm ref="refVForm" @submit.prevent="sendMessage">
                            <VTextField v-model="message" placeholder="Type your message..."
                                :rules="[v => !!v || 'Message is required']" hide-details density="comfortable"
                                class="message-input">
                                <template #append>
