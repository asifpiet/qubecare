...
const headers = [
    { title: 'id', key: 'id' },
    { title: 'Commission', key: 'total_commission' },
...
const datatableUrl = ref(GET_PARTNER_MONTHLY_STATEMENTS)
...
watch(selectedMonth, (newMonth) => {
  ...
  datatableUrl.value = `${GET_PARTNER_MONTHLY_STATEMENTS}?start_date=${startDate}&end_date=${endDate}`;
  reload();
});
...
<template>
    ...
    <DataTable ref="dataTable" :cardTitle="$t('Monthly Report')" btnTitle="" :headers="headers" :api="datatableUrl"
        ...
        <template #item.provider_name="{ item }">
            <span>{{ getProviderName(item.statement_items) }}</span>
        </template>

        <template #item.statement_date="{ item }">
            <span>{{ dateFormat(item.Statement_date) }}</span>
        </template>
