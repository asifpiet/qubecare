...
onBeforeMount(async () => {
    const questionereFormIds = products.filter(product => product.in_take_form_id != null && product.in_take_form_id !== 0).map(product => product.in_take_form_id);
    const catQuestionereFormIds = products.filter(product => product.category_intake_form_id != null && product.category_intake_form_id !== 0).map(product => product.category_intake_form_id);
    const combinedUniqueArray = [...new Set([...questionereFormIds, ...catQuestionereFormIds, ...builderData.intakes])].filter(item => item != null);
...
const onNext = async (props) => {
    ...
    await store.dispatch('storePatientIntakeFormData', {
        form_id: item.id,
        pid: patientData.id,
    ...
    <VRow style="min-height: 101.7dvh; max-height: 101.7dvh; margin: 0px;"
        ...
        <VCol cols="12" md="12" class="bg-custom col-order-1"
            ...
            <v-stepper v-model="step" prev-text="Previous" next-text="Next" :items="items" item-title="title">
                <template #item="item">
                    <IntakeForm :formId="item.raw.id" :formRef="formsRef[item.raw.id]"></IntakeForm>
                </template>
