...
onMounted(async () => {
    ...
    providerStore.getTimeLineReport();
});
...
<template>
    ...
    <VCard class="mt-5">
        <VCardItem>
            <VCardTitle> {{ $t('Treatment Timeline') }} </VCardTitle>
        </VCardItem>

        <VCardText>
            <VTimeline v-if="providerStore.timeLineReport && providerStore.timeLineReport.length > 0" side="end"
                align="start" line-inset="8" truncate-line="start" density="compact">
                <VTimelineItem v-for="(item, index) in providerStore.timeLineReport" :key="index" dot-color="primary"
                    size="x-small">
                    <!-- 👉 Header -->
                    ...
                    <div class="app-timeline-text mt-1">
                        {{ $t('Order Status') }} : {{ item.OrderStatus }}<br />
                        {{ $t('Patient') }} : {{ item.patient_name }}<br />
                        {{ $t('Medication') }} : <VChip size="small" color="primary" class="me-2">
                            {{ item.OrderMedication.join(', ') }}
                        </VChip>
                    </div>
                </VTimelineItem>
            </VTimeline>
            ...
            <div v-else class="text-center">
                <p class="text-muted"> {{ $t('No timeline data available for the selected dates.') }} </p>
            </div>
