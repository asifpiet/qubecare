...
const editProduct = computed(() => {
  ...
  selectCategoryId.value = props.editProduct.category_ids;
...
<template>
  ...
  <VNavigationDrawer temporary :width="800" location="end" class="scrollable-content" :model-value="props.isDrawerOpen"
    ...
    <AppDrawerHeaderSection title="Edit Product" @cancel="closeNavigationDrawer" />

    <VDivider />
    {{ props.editProduct.in_take_form_id }}
      ...
      <VCard flat>
        <VCardText>
          <v-form ref="refForm">
            <v-row>
              <v-col cols="12" md="6">
                <v-text-field v-model="name" :rules="[requiredValidator]" label="Name" outlined required
                  @input="generateSlug"></v-text-field>
              </v-col>

              <v-col cols="12" md="6">
                <v-text-field v-model="slug" label="Slug" outlined readOnly></v-text-field>
              </v-col>
            ...
            <v-row>
              ...
              <v-col cols="12" md="6">
                <v-select v-model="selectCategoryId" :items="categories" label="Category" outlined required
                  item-title="name" item-value="id"></v-select>
              </v-col>
              <v-col cols="12" md="6">
                <VTextField v-model="price" type="number" label="Price" @blur="formatPrice" @input="removeFormatting" />
              </v-col>
              <v-col cols="12" md="6">
                <v-select v-model="type" :items="priceOptions" label="Type" outlined required></v-select>
              </v-col>



              <v-col cols="12" md="6">
                <v-select v-model="inTakeForm" :items="inTakeOptions" label="Intake Form" outlined required
                  item-title="name" item-value="id"></v-select>
              </v-col>

              <v-col cols="12" md="6">
                <v-select v-model="questiioneriesForm" :items="questionnaireOptions" label="Questionnaire Form" outlined
                  required item-title="name" item-value="id"></v-select>
              </v-col>
              <v-col cols="12" md="6">
                <v-text-field v-model="sku" label="SKU" outlined></v-text-field>
              </v-col>
              <v-col cols="12" md="6">
                <v-checkbox v-model="isPrescription" label="Requires Prescription" outlined></v-checkbox>
              </v-col>

              <v-col cols="12" md="6">
                <VSelect v-model="status" label="Status" :items="['Active', 'Inactive']"></VSelect>
              </v-col>
            </v-row>
            <v-row>
              <v-col cols="12">
                <v-textarea v-model="description" label="Description" outlined rows="4"></v-textarea>
              </v-col>
            </v-row>
            <v-row>
