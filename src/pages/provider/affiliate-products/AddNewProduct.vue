...
<template>
  <VNavigationDrawer temporary :width="800" location="end" class="scrollable-content"
    ...
    <AppDrawerHeaderSection
      :title="affiliateProductStore.id ? $t('Edit Affiliate Network') : $t('Add To Affiliate Network')"
        ...
        <VCardText>
          <v-form ref="refForm">
            <v-row>
              ...
              <v-col cols="12" md="12">
                <v-select v-model="affiliateProductStore.affiliateProduct.product_id" :items="products"
                  :label="$t('Products')" outlined required item-title="name" item-value="id"></v-select>
              </v-col>
              <v-col cols="12" md="12">
                <v-select v-model="affiliateProductStore.affiliateProduct.builder_id" :items="builders"
                  :label="$t('Builder')" outlined required item-title="builder_name" item-value="builder_id"></v-select>
              </v-col>
              <v-col cols="12" md="12">
                <VTextField v-model="affiliateProductStore.affiliateProduct.referral_fee" type="number"
                  :label="$t('Referral Fee')" @blur="formatPrice" @input="removeFormatting" />
              </v-col>

              <v-col cols="12" md="6">
                <v-checkbox v-model="affiliateProductStore.affiliateProduct.status" :label="$t('Active')"
                  outlined></v-checkbox>
              </v-col>
            </v-row>
            <v-row>
