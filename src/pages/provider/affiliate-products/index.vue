...
import AddNewProduct from '@/pages/provider/affiliate-products/AddNewProduct.vue';

const affiliateProductStore = useAffiliateProductStore()
const sortByData = ref([{ 'key': 'affiliate_product_id', 'order': 'desc' }])
...
const headers = [
  {
    title: 'ID',
    key: 'affiliate_product_id',
    name: 'affiliate_products.id',
  },
  {
    title: 'Product',
    key: 'product_name',
  ...
  },
  {
    title: 'Status',
    key: 'status',
    name: 'affiliate_products.status',
  },
...
<template>
  <section>
    <DataTable ref="dataTable" :cardTitle="$t('Affiliate Products')" :btnTitle="$t('Add To Affiliate Network')"
      :headers="headers" :api="GET_AFFILIATE_PRODUCTS" :sortBy="sortByData"
