...
onMounted(async () => {
    if (code) {
        ...
        loading.value = false
        // try {
        //     let responseCalback = await $api(GET_HUBSPOT_CALLBACK + '/code=' + code + '/state/' + state, {
        //         method: 'GET',
        ...
        //     if (responseCalback.status === 'success') {
        //         gConnected.value = true
        //         loading.value = false
...
const connectGoogle = async () => {
    loading.value = true
    try {
        let responseRedirect = await $api(GET_HUBSPOT_AUTH, {
            method: 'GET',
            onResponseError({ response }) {
                console.error('Error in API:', response);
            }
        ...
        window.open(responseRedirect.data.auth_url, '_blank');
        loading.value = false
    } catch (error) {
        loading.value = false
        addAlert('Something went wrong', 'error')
...
const getGoogleSettings = async () => {
    gTable.value = []
    let responseSettings = await $api(GET_HUBSPOT_STATUS, {
        method: 'GET',
        onResponseError({ response }) {
            console.error('Error in API:', response);
        }
    ...
    gConnected.value = responseSettings.connected
...
const gDisconnectSync = async () => {
    ...
    try {
        ...
        gConnected.value = false
        loading.value = false
        addAlert('Successfully Disconnected', 'success')
    } catch (error) {
        loading.value = false
        addAlert('Something went wrong', 'error')
        console.error('Error in API:', error.response || error.message || error);
...
<template>
    ...
    <v-navigation-drawer v-model="drawer" temporary :width="$vuetify.display.smAndDown ? 'auto' : 500" location="end">
        <div v-if="gConnected">
            <AppDrawerHeaderSection title="Hubspot" @cancel="drawer = false" />
            <v-divider />
        ...
        <div v-if="!gConnected">
            <AppDrawerHeaderSection :title="$t('Connect your Hubspot')" @cancel="drawer = false" />
            <v-divider />
            <p class="mt-2" style="padding: 20px;">
                {{ $t('You will soon be prompted to connect your Hubspot account with') }}
    ...
    <VCard :title="$t('External Hubspot Sync')"
        :subtitle="$t('Click on the button bellow to connect and sync the hubspot.')">
