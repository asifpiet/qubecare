...
<template>
  ...
  <VCardText>
    <VForm class="mt-2" ref="refVForm">
      <VRow>
        ...
        <VCol cols="6">
          ...
          <AppSelect v-model="userType" :label="$t('User Type')"
            ...
            item-title="title" item-value="value" :placeholder="$t('Select User Type')" :rules="[requiredValidator]" />
        </VCol>
        <VCol cols="6">
      ...
      <div class="notes">
        <p> {{ $t('Notes') }}:</p>
        <p>{{ $t(`Passwords can be set/reset by email or text on the Login page. An admin will have access to patient
          consult records.`) }}
        </p>
      </div>
    </VForm>
