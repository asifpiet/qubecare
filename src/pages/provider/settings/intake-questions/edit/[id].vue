...
await formsStore.getIntakeFormData(id)
onMounted(async () => {
    data.value = formsStore.intakeQuestion
    name.value = data.value?.name
    description.value = data.value?.description
    status.value = data.value?.status == 1 ? true : false

    makeFormeoForm(jsonConvert(deepToRaw(formsStore.intakeQuestion.data)))
...
<template>
    <VCard :title="$t('Edit Intake Questions')">
        <VCardText>
