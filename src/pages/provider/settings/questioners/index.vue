...
const addNewItem = () => {
  router.push({ name: 'provider-settings-questioners-add-questioners' });
};

const editItem = (item) => {
  router.push({
    name: 'provider-settings-questioners-edit-id',
    params: { id: item.id } // Use item.id here
  });
};

const deleteItem = async (item) => {
  await formsStore.deleteQuestionerQuestionsForm(item.id)
  reload()
};
</script>

<template>
  <DataTable ref="dataTable" cardTitle="Questionnairs" btnTitle="Add New Questionnaire" :headers="headers"
    :api="GET_QUESTIONER_QUESTIONS" :sortBy="sortByData" @onBtnClick="addNewItem">
