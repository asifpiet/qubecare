    ...
    refVForm.value?.validate().then(async ({ valid: isValid }) => {
        ...
        if (isValid) {
            ...
            try {
                ...
                await providerStore.resetPassword();
                ...
                <VCardText>
                    <VForm ref="refVForm" @submit.prevent="onSubmit">
                        <VRow>
                            <!-- email -->
                            <VCol cols="12">
                                <AppTextField v-model="providerStore.passwordReset.verification_code"
                                    :rules="[requiredValidator, verificationCodeValidator]"
                                    :error-messages="errors.verification_code" autofocus :label="$t('Passcode')"
                                    type="number" maxlength="6" />
                            </VCol>

                            <VCol cols="12">
                                <VTextField v-model="providerStore.passwordReset.password" :label="$t('Password')"
                            ...
                            <VCol cols="12">
                                <VTextField v-model="providerStore.passwordReset.password_confirmation"
                                    :label="$t('Confirm Password')" placeholder="············"
                                    :rules="[requiredValidator]" :type="isConfirmPasswordVisible ? 'text' : 'password'"
                            ...
                            <!-- back to login -->
                            <VCol cols="12">
                                <RouterLink class="d-flex align-center justify-center" :to="'/provider/login'">
