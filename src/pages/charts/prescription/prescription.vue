...
onMounted(async () => {
  ...
  await store.dispatch('updateIsLoading',true)
      const now = new Date()
        ...
        await store.dispatch('getPrescriptionReport',{
          start_date: start_date.value,
          end_date: end_date.value,
        });
        prescriptionReport.value  = store.getters.getPrescriptionReport;
        console.log("prescriptionReport",prescriptionReport.value);
   ...
   try {
     ...
     const [startDate, endDate] = dateString.split(" to ");
      ...
      if (startDate && endDate) {
        ...
        prescriptionReport.value  = store.getters.getPrescriptionReport;
          console.log("getPrescriptionReport",prescriptionReport.value);  
        await store.dispatch('updateIsLoading',false)
     } else {
...
<template>
  ...
  <VCard class="mt-5">
    <VCardItem>
      <VCardTitle>Prescription History</VCardTitle>
    </VCardItem>

    <VCardText>
        <VTimeline
  v-if="prescriptionReport && prescriptionReport.length > 0"
  side="end"
  align="start"
  line-inset="8"
  truncate-line="start"
...
>
  <VTimelineItem
    v-for="(item, index) in prescriptionReport"
    :key="index"
    dot-color="primary"
    size="x-small"
  >
...
<div v-else class="text-center ">
  <p class="text-muted">No Prescription data available for the selected dates.</p>
</div>
