...
patientRegStore.currentPage = 'Product Form'
...
const intakeFormIds = products.filter(product => product.in_take_form_id != null && product.name !== 'Doctor Visit' && product.in_take_form_id !== 0).map(product => product.in_take_form_id);
const questionereFormIds = products.filter(product => product.questiioneries_form_id != null && product.name !== 'Doctor Visit' && product.questiioneries_form_id !== 0).map(product => product.questiioneries_form_id);
...
const onCompleted = (() => {
    console.log('step.value', step.value, items.value.length)
