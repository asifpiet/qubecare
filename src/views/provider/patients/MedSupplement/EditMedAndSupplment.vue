...
<template>
  <section>
    <DataTable ref="dataTable" cardTitle="" :btnTitle="$t('Add Prescription')" :headers="headers"
      :api="GET_PRESCRIPTION_LIST + '/' + props.patient.id" :sortBy="sortByData"
      ...
      <template #item.actions="{ item }">
        ...
        <IconBtn @click="history(item.id)" v-if="item.status !== 'pending' && item.status !== 'Reorder Pending'">
          ...
          <v-form ref="refForm" v-model="isFormValid" @submit.prevent="onSubmit('image/jpeg')">
            ...
            <v-row>
              ...
              <v-col cols="12">
                ...
                <v-radio-group v-model="medicationStore.needs_followup" inline :rules="[requiredFieldValidator]">
                  <v-radio :label="$t('Yes')" value="Yes" />
                  <v-radio :label="$t('No')" value="No" />
                </v-radio-group>
              </v-col>
              <v-col cols="12" v-if="medicationStore.needs_followup == 'Yes'">
                <AppTextField v-model="medicationStore.numberOfDays" type="number" :label="$t('Number Of Days')"
                  :placeholder="$t('Number Of Days')" @input="allowPositiveNumbers" :rules="[requiredValidator]" />
              </v-col>
              <VCol cols="12">
