...
<script setup>
definePage({ meta: { layoutWrapperClasses: 'layout-content-height-fixed', action: 'read',
...
import { nextTick, onMounted, ref, watch } from 'vue';
  ...
  window.Pusher = Pusher;
    ...
    let echo = new Echo({
        ...
        auth: {
            headers: {
                Authorization: 'Bearer 230|xntUP4MXyF6jV2luiBfiUwKtLGJYu3qAdqm2dpGjb642717e' ,
            },
        },
    });
  watch(
  ...
  const fetchMessages = async () => {
      try {
      const response = await axios.get(`http://127.0.0.1:8000/api/messages/${props.user.id}`);
