...
const isValidatingZipcode = ref(false);
...
onMounted(async () => {
  console.log('patientStore.demographic.primaryPractitioner', patientStore.demographic.primaryPractitioner)
  ...
  if (patientStore.demographic.profilePicture) {
    console.log("profilePicture", patientStore.demographic.profilePicture);
...
const changeAvatar = async (event) => {
  ...
  if (files && files.length) {
    ...
    fileReader.onload = async () => {
      ...
      patientStore.demographic.avatar = fileReader.result;
...
const onSubmit = async () => {
  ...
  return refForm.value?.validate().then(async ({ valid }) => {
    if (valid) {
      ...
      return patientStore.saveDemographic(formData);
...
// Function to verify zipcode against address
const verifyZipcode = async () => {
  // Only validate if all required fields are filled
  if (!patientStore.demographic.address ||
    !patientStore.demographic.city ||
    !patientStore.demographic.state ||
    !patientStore.demographic.zipcode) {
    return;
  }

  isValidatingZipcode.value = true;

  try {
    // Construct the full address
    const fullAddress = `${patientStore.demographic.address}, ${patientStore.demographic.city}, ${patientStore.demographic.state}, ${patientStore.demographic.zipcode}`;

    // Call Google Geocoding API
    const response = await fetch(`https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(fullAddress)}&key=AIzaSyBdqGrpK1HLNCZJueD7g3uUlKcLEDM_MmQ`);
    const data = await response.json();

    if (data.status === 'OK') {
      // Extract zipcode from Google's response
      const addressComponents = data.results[0].address_components;
      const googleZipcode = addressComponents.find(component =>
        component.types.includes('postal_code')
      )?.long_name;

      if (googleZipcode && googleZipcode !== patientStore.demographic.zipcode) {
        zipcodeError.value = "Your zipcode does not match the address provided";
      } else {
        zipcodeError.value = '';
      }
    } else {
    ...
    isValidatingZipcode.value = false;
  }
};

// Custom validation rule for zipcode
const validateZipcode = (value) => {
  // Basic zipcode format validation
  if (!/^\d{5}(-\d{4})?$/.test(value)) {
    return 'Please enter a valid zipcode';
  }

  // Return custom error if zipcode doesn't match address
  if (zipcodeError.value) {
    return zipcodeError.value;
  }

  return true;
};

// Set up watcher for zipcode changes
watch(() => patientStore.demographic.zipcode, (newValue, oldValue) => {
  if (newValue && newValue !== oldValue) {
    // Reset error when user changes zipcode
    zipcodeError.value = '';
  }
});

// Optional: You can also watch the entire address to validate
// when any component changes
const addressComponents = computed(() => ({
  address: patientStore.demographic.address,
  city: patientStore.demographic.city,
  state: patientStore.demographic.state,
  zipcode: patientStore.demographic.zipcode
}));

watch(addressComponents, () => {

  // Optional: Automatically validate when all fields are filled
  if (addressComponents.value.address &&
    addressComponents.value.city &&
    addressComponents.value.state &&
    addressComponents.value.zipcode) {
    verifyZipcode();
  }
}, { deep: true });
</script>
<template>
  ...
  <VCardText class="d-flex pl-0 pt-0">
    ...
    <VAvatar rounded size="115" class="me-5"
      :image="patientStore.demographic.profilePicture ? patientStore.demographic.profilePicture : accountDataLocal.avatarImg" />
  ...
  <VForm ref="refForm" v-model="isFormValid" @submit.prevent="onSubmit">
    <VRow>
      ...
      <VCol cols="6">
        <AppTextField v-model="patientStore.demographic.lastName" :rules="[requiredValidator]" :label="$t('Last Name')"
          class="field-required" :placeholder="$t('Last Name')" />
      </VCol>
      ...
      <!-- 👉 Email -->
      <VCol cols="6">
        <AppTextField v-model="patientStore.demographic.email" :rules="[requiredValidator, emailValidator]"
          :label="$t('Email')" class="field-required" :placeholder="$t('Email')" />
      </VCol>
      ...
      <VCol cols="6">
        <AppTextField v-model="patientStore.demographic.dateOfBirth" type="date" :rules="[requiredValidator]"
          :label="$t('Date Of Birth')" class="field-required" :placeholder="$t('Date Of Birth')" />
      </VCol>
      ...
      <VCol cols="6">
        <AppSelect v-model="patientStore.demographic.pronoun" :label="$t('Pronoun')" :placeholder="$t('Pronoun')"
          :items="['He', 'She', 'They', 'Unknown']" />
      </VCol>
      <!-- <VCol cols="6">
        <AppSelect v-model="patientStore.demographic.AgeGroup" :label="$t('Age Group')" :placeholder="$t('Age Group')"
          :items="['Child', 'Adult']" />
      </VCol> -->

      <VCol cols="6">
        <AppTextField v-model="patientStore.demographic.preferredPhone" pattern="^\(\d{3}\) \d{3}-\d{4}$"
          :rules="[requiredValidator, validUSAPhone]" :label="$t('Preferred Phone')" class="field-required" max="14"
      ...
      <VCol cols="6">
        <AppTextField v-model="patientStore.demographic.address" :rules="[requiredValidator]" :label="$t('Address')"
          class="field-required" :placeholder="$t('Address')" />
      </VCol>
      <VCol cols="6">
      ...
      <VCol cols="6">
        <AppSelect v-model="patientStore.demographic.state" :label="$t('State')" class="field-required"
          :placeholder="$t('State')" :rules="[requiredValidator]" :items="states" item-title="name"
          item-value="abbreviation" />
      </VCol>
      <VCol cols="6">
        <AppTextField v-model="patientStore.demographic.zipcode" type="number"
          :rules="[requiredValidator, validateZipcode]" :label="$t('Zipcode')" class="field-required"
          :placeholder="$t('Zipcode')" @blur="verifyZipcode" :error-messages="zipcodeError" />
      </VCol>
      <!-- <VCol cols="6">
                <AppSelect
      ...
      <VCol cols="6">
        ...
        <AppSelect v-model="patientStore.demographic.primaryPractitioner" :rules="[requiredValidator]"
          :label="$t('Primary Practitioner')" class="field-required" :placeholder="$t('Primary Practitioner')"
      ...
      <VCol cols="6">
        <AppTextField v-model="patientStore.demographic.primaryCarePhysician" :label="$t('Primary Care Physician')"
          :placeholder="$t('Primary Care Physician')" />
      </VCol>

      <VCol cols="6">
        <AppTextField v-model="patientStore.demographic.guardian" :label="$t('Guardian Name')"
          :placeholder="$t('Guardian')" />
      </VCol>

      <VCol cols="6">
        <AppTextField v-model="patientStore.demographic.emergencyContactNumber" pattern="^\(\d{3}\) \d{3}-\d{4}$"
          :rules="[validUSAPhone]" :label="$t('Emergency Contact Number')" max="14"
      ...
      <VCol cols="6">
        <AppTextField v-model="patientStore.demographic.emergencyContactNameRelation"
          :label="$t('Emergency Contact Name')" :placeholder="$t('Emergency Contact Name')" />
      </VCol>
