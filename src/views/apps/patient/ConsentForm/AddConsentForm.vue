...
let practitionerId = props.selectedPractitionerId;
const data = ref({})
...
onMounted(async () => {
    ...
    tempData = jsonConvert(deepToRaw(formsStore.consentFormData.data))
...
const closeModalDrawer = () => {
    emit('isConsentDrawerOpen', false)
}

const save = async (t) => {
    signature.value = signatureRef.value.save(t)
    console.log(signatureRef.value.save(t))
    formData = new FormData(consentForm.value)
    console.log('formData', Object.fromEntries(formData.entries()), renderer)
    await formsStore.savePatientConsentForm(
        {
            form_id: formId,
...
<template>
  ...
  > -->
    ...
    <!-- <VCard flat>
        <VCardText>   -->
    <form ref="consentForm" @submit.prevent="save('image/jpeg')">
        <VRow>
            <VCol cols="12">
            ...
            <VCol cols="12">
                <div v-html="description"
                    style="border: 1px solid silver;border-radius: 5px;min-height: 200px;padding: 5px;"></div>
            </VCol>
            ...
            <VCol cols="12">
                <AppTextField v-model="yourName" label="Your Name" placeholder="" :rules="[requiredValidator]" />
            </VCol>

            <VCol cols="3">
                <Vue3Signature style="border: 1px solid silver; border-radius: 5px;" ref="signatureRef"
                    :sigOption="state.option" :w="'auto'" :h="'50px'" :disabled="state.disabled" class="example">
                </Vue3Signature>
                <VBtn @click="clear" color="secondary" size="x-small">Clear</VBtn>
            </VCol>
