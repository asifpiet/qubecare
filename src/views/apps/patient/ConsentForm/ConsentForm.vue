...
<template>
  <VNavigationDrawer temporary :width="800" location="end" class="scrollable-content" :model-value="props.isDrawerOpen"
    ...
    <AppDrawerHeaderSection :title="$t('Consent Form') + ' (' + props.patient.demographic.fullName + ')'"  
      @cancel="closeNavigationDrawer" />

    <PerfectScrollbar :options="{ wheelPropagation: false }">

      <VCardText>
        <DataTable ref="dataTable" cardTitle="" :btnTitle="$t('New Consent Form')" :headers="headers"
      ...
      <VNavigationDrawer v-model="isSelectLabDialogVisible" temporary :width="800" location="end"
        ...
        <AppDrawerHeaderSection :title="$t('Add Consent') + ' (' + props.patient.demographic.fullName + ')'" 
        ...
        <!-- <VCard title="Please Consent Form"> -->
          ...
          <VCardText>
            <VRow>
              <VCol cols="5">
                <AppSelect v-model="practitioner" :label="$t('Select Practitioner')" placeholder="" :items="practitioners"
                  item-value="id" item-title="name" :rules="[requiredValidator]" />
              </VCol>
              <VCol cols="5">
                <AppSelect v-model="consentFormId" :label="$t('Select Consent Form')" placeholder="" :items="formsStore.forms"
                  item-value="id" item-title="name" :rules="[requiredValidator]" />
              </VCol>
              <VCol cols="2">
                <VBtn class="mt-6" @click="openAddModal">{{ $t('Select')}}</VBtn>
              </VCol>
            </VRow>
          </VCardText>
        <!-- </VCard> -->
      ...
      <VNavigationDrawer v-model="isAddConsentFormDrawerVisible" temporary :width="800" location="end"
        ...
        <!-- <VCard title=""> -->
          <VDivider/>
          <VCardText>
            <Intake @closePopup="isConsentDrawerOpen" :patient="props.patient" :selectedFormId="consentFormId" v-if="consentFormId"
      ...
      <VNavigationDrawer v-model="isPdfConsentFormDrawerVisible" temporary :width="800" location="end"
        class="scrollable-content">
        <AppDrawerHeaderSection :title="$t('PDF Consent') + ' (' + props.patient.demographic.fullName + ')'"
          ...
          <PerfectScrollbar :options="{ wheelPropagation: false }" v-if="intakefieldValue && intakefieldValue.pdf_url">
            <PdfViewr v-if="intakefieldValue" :pdfUrl="intakefieldValue.pdf_url" v-model:isOpen="isPdfConsentFormDrawerVisible"
              :title="intakefieldValue.name" />
          </PerfectScrollbar>
      </VNavigationDrawer>
</PerfectScrollbar>
  </VNavigationDrawer>
