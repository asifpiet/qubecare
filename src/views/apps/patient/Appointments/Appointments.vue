...
const loadItems = debounce(async ({ page, itemsPerPage, sortBy }) => {
  ...
  loading.value = true;
  const data = await API.getDataTableRecord(GET_APPOINTMENTS + '/' + props.patient.id, payload, headers);
...
onMounted(async () => {
  //await store.dispatch('getAppointmentsList',{id: props.patient.patientId});
...
const deleteAppointments = async vid => {
  ...
  await store.dispatch('getAppointmentsList', { id: props.patient.id });
  appointments.value = store.getters.getAppointments
};
...
<template>
  ...
  <VNavigationDrawer temporary :width="800" location="end" class="scrollable-content" :model-value="props.isDrawerOpen"
    ...
    <AppDrawerHeaderSection :title="$t('Appointments') + ' (' + props.patient.demographic.fullName + ')'"
        ...
        <VCardText class="d-flex flex-wrap gap-4">
          <div class="app-user-search-filter d-flex align-center flex-wrap gap-4">
            ...
            <div style="inline-size: 15.625rem;">
              <AppTextField v-model="search" :placeholder="$t('Search')" />
            </div>
          </div>
          <VDivider />
          <VDataTableServer v-model:items-per-page="itemsPerPage" :headers="translatedHeaders" :items="serverItems"
