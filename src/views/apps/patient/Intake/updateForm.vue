...
onMounted(async () => {
  console.log('props.selectedFormId', props.selectedFormId, props.rowId)
  isLoading.value = true
  if (props.rowId == 0) {
    await formsStore.getIntakeFormDataById(id)

  } else {
    await formsStore.getPatientIntakeFormsData(id, props.patient.id, props.rowId)
    fdata = formsStore.patientIntakeFormData
    console.log('ccc', fdata)
    if (!fdata.patient_custom_fname || !fdata.patient_custom_fname) {
      patientStore.loadPatient(props.patient.id)
      fdata.patient_custom_fname = patientStore.demographic.firstName
      fdata.patient_custom_lname = patientStore.demographic.lastName
    }
  }
  ...
  Object.keys(tempData.fields).forEach((key) => {
    const field = tempData.fields[key];
    evaluateConditions(field, tempData.fields, refVForm.value);
  });
...
const submitForm = async () => {
  ...
  try {
    await formsStore.updatePatientIntakeForm(formData, props.rowId)
    ...
    <VCardText>
      ...
      <form ref="intakeSettingForm" @submit.prevent="submitForm" enctype="multipart/form-data">
        ...
        <VRow class="mt-2" v-if="!isLoading">
          <VCol cols="4">
            <VBtn type="submit" class="submit-button">Save</VBtn>
          </VCol>
        </VRow>
      </form>
    </VCardText>
