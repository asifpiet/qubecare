...
<template>
  <DataTable ref="dataTable" cardTitle="" :btnTitle="$t('Form Request')" :headers="headers"
    :api="GET_PROVIDER_REQUEST_FORM + '/' + props.patient.id + '/charting-forms'" :sortBy="sortByData"
  ...
  <VDialog v-model="isIntakeDrawerVisible" max-width="1000">
    <DialogCloseBtn @click="[isIntakeDrawerVisible = false, dataTable.reload()]" />
    <VCard :title="formName">
      <VCardText>
  ...
  <VNavigationDrawer v-model="isSelectLabDialogVisible" temporary :width="800" location="end"
    ...
    <AppDrawerHeaderSection :title="$t('Form Request') + ' (' + props.patient.demographic.fullName + ')'" 
  ...
  <VNavigationDrawer v-model="isViewConsentFormDrawerVisible" temporary :width="800" location="end"
    class="scrollable-content">
    <AppDrawerHeaderSection :title='"View Request (" + props.patient.demographic.fullName + ")"'
