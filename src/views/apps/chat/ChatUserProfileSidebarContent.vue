...
const translatedHeaders = computed(() => {
  return userStatusRadioOptions.map(header => ({
    ...header,
  ...
  <template v-if="store.profileUser">
    ...
    <PerfectScrollbar class="ps-chat-user-profile-sidebar-content pb-5 px-6" :options="{ wheelPropagation: false }">
      ...
      <div class="my-6 text-medium-emphasis">
        ...
        <AppTextarea id="textarea-user-about" v-model="store.profileUser.about" auto-grow class="mt-1" rows="3" />
      </div>
      ...
      <div class="mb-6">
        ...
        <VRadioGroup v-model="store.profileUser.status" class="mt-1">
          <VRadio v-for="radioOption in translatedHeaders" :key="radioOption.title" :label="radioOption.title"
            :value="radioOption.value" :color="radioOption.color" />
        </VRadioGroup>
      </div>
