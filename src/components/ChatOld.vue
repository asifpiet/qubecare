...
const initPusherListeners = async () => {
  ...
  if (currentUser.value.user_type === 'patient') {
    ...
    // Patient listening for messages from practitioner
    window.Echo.private(`message-${currentUser.value.id}-${activeSubject.value.id}`)
      .listen('OnNewMessage', async (event) => {
        //console.log('New message received: patient ' + JSON.stringify(event));
        console.log(`Listening to channel: message-${currentUser.value.id}-${activeSubject.value.id}`);
        console.log(`Message intended for patient: ${event.patient_id}, current patient: ${activeSubject.value.id}`);
        console.log(`Message from practitioner: ${event.practitioner_id}, active practitioner: ${currentUser.value.id}`);
        // Ensure the message is intended for the current patient
        if (event.patient_id === currentUser.value.id && event.practitioner_id === activeSubject.value.id) {
    ...
    //window.Echo.leave(`message-${currentUser.value.id}-${activeSubject.value.id}`)
  } else {
    // Practitioner listening for messages from patient
    window.Echo.private(`message-${activeSubject.value.id}-${currentUser.value.id}`)
      .listen('OnNewMessage', async (event) => {
        // console.log('New message received user: ' + JSON.stringify(event));
        console.log(`Listening to channel: message-${activeSubject.value.id}-${currentUser.value.id}`);
        console.log(`Message intended for patient: ${event.patient_id}, current patient: ${activeSubject.value.id}`);
        console.log(`Message from practitioner: ${event.practitioner_id}, active practitioner: ${currentUser.value.id}`);
        // Ensure the message is intended for the current practitioner
        if (event.practitioner_id === currentUser.value.id && event.patient_id === activeSubject.value.id) {
          const messageExists = messageStore.messageList.some(val => val.id === event.id);
    ...
    //window.Echo.leave(`message-${activeSubject.value.id}-${currentUser.value.id}`)
  }
...
onMounted(async () => {
  ...
  if (props.patient) {
    const fieldIndexInMerged = messageStore.usersList.findIndex(val => val.id === props.patient.id);
    openSubjectMessages(messageStore.usersList[fieldIndexInMerged])
  }
...
const openSubjectMessages = async (subject) => {
  ...
  await messageStore.getMessagesByUser(subject.id)
  console.log(messageStore.messageList)
  scrollToBottom()
  await initPusherListeners()
  await messageStore.markMessagesAsRead(subject.id)
}
...
const isCurrentUser = (messageSender) => {
  return messageSender === currentUser.value.user_type
}
</script>
    ...
    <div class="messages-area" style="flex-grow: 1; display: flex; flex-direction: column;">
      <template v-if="activeSubject">
        ...
        <PerfectScrollbar ref="messageContainer" class="custom-scrollbar messages-container">
          ...
          <div class="messages-content" style="height: calc(100vh - 54px);" v-if="messageStore.messageList">
        ...
        <div class="message-input px-4 py-3 border-top mb-3">
          <VForm ref="refVForm" @submit.prevent="sendMessage">
            <VTextField v-model="message" placeholder="Type your message..."
